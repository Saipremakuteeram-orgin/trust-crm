...
function buildWorkbook(txns, profile) {
  ...
  sheet.columns = [
    ...
    { key: 'voucher_filed', width: 12 },
    { key: 'notification_status', width: 14 },
  ];

  const headerRow = sheet.getRow(dataStartRow);
  headerRow.values = ['Date', 'Type', 'Mode', 'Amount (₹)', 'Party', 'Category', 'Description', 'Reference', 'Voucher', 'Notification'];
  ...
  sheet.autoFilter = { from: 'A' + dataStartRow, to: 'J' + dataStartRow };
