...
export default function ProfileMenu() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-saffron-400 to-saffron-600 flex items-center justify-center text-white text-[11px] font-bold shadow-sm">
          {initials}
        </div>
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl shadow-stone-300/40 border border-stone-100 py-2 z-50"
          style={{ top: "calc(100% + 0.5rem)" }}
        >
          <div className="px-4 py-3 border-b border-stone-100">
            <div className="text-sm font-semibold text-stone-800 truncate">{profile?.full_name || "Loading…"}</div>
            <div className="text-[11px] text-stone-500 truncate">{profile?.email || ""}</div>
